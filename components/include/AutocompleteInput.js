...
function AutocompleteInput(props) {
  ...
  function onChange(e) {
    ...
    let filteredSuggestions = suggestions.filter(
      (suggestion) =>
        suggestion.ms_name.toLowerCase().indexOf(userInput.toLowerCase()) >
          -1 ||
        (suggestion.ms_address &&
          suggestion.ms_address.toLowerCase().indexOf(userInput.toLowerCase()) >
            -1) ||
        (suggestion.ms_postal &&
          suggestion.ms_postal.toLowerCase().indexOf(userInput.toLowerCase()) >
            -1)
    );

    //calculate distance
    filteredSuggestions.map((item) => {
      item.distance = Utils.getAcademyDistance(item, defaultLocation, "mile");
      return true;
    });
    ...
    if (filteredSuggestions.length || predict.length) {
              ...
              >
                <div className={className}>
                  <div className="suggest-title">
                    <label className="title-text">{suggestion.ms_name}</label>
                    <div>
                      <label className="distance-text">
                        {`${suggestion.distance} miles`}
                      </label>

                      <Link to={"/" + suggestion.ms_alias}>
                        <FontAwesomeIcon
                          icon={faChevronRight}
                          style={{
                            color: "#EE7925",
            ...
            return (
              <li
                ...
                onClick={(e) => {
                  ...
                  fetch(proxyurl + detailPlaceUrl)
                    ...
                    .then((data) => {
                      if (data.status === "OK") {
                        let placeLocation = data.result.geometry.location;
                        props.selectAcademy({
                          ms_latitude: placeLocation.lat,
                          ms_longitude: placeLocation.lng,
                        });
              ...
              >
                <div className="suggest-item">
                  ...
                  <label className="sub-title">{item.description}</label>
                </div>
              </li>
            );
          })}
      ...
      suggestionsListComponent = (
        <div className="no-suggestions">
          ...
          <em>
            There aren't any academies related to your search. Could you setup a{" "}
            <a style={{ color: "red" }} href="https://franchisewmf.com/">
              WMF franchise
            </a>{" "}
            at this location?
          </em>
          ...
          <div className="form-input">
            <div className="text-input">
              ...
              <svg
                onClick={() => {
                  props.selectAcademy({
                    ms_latitude: localStorage.getItem("latitude"),
                    ms_longitude: localStorage.getItem("longitude"),
                  });
                }}
                width="18px"
                height="20px"
