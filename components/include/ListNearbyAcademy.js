...
import { headerActionType, siteActionType } from "redux/actionTypes";
...
function ListNearbyAcademy() {
  ...
  useEffect(() => {
    ...
    if (!defaultAcademy && !!lstNearest[0]) {
      localStorage.setItem("defaultAcademy", JSON.stringify(lstNearest[0]));
      dispatch({
      ...
      {lstAcademy.length > highlightAcademy && (
        <div className="main-info wrap-row">
          <div className="wrap-address">
            ...
            <label>
              {Utils.showDistance(lstAcademy[highlightAcademy].distance)}
              {` miles away`}
            </label>
          ...
          <div className="wrap-info">
            <img
              ...
              src={
                ...
                lstAcademy[highlightAcademy].ms_avatar !== ""
                  ? Utils.getThumb(lstAcademy[highlightAcademy].ms_avatar, "c1")
                  : avatar
              }
            />
          </div>
            ...
            <a
              ...
              onClick={() => {
                localStorage.setItem(
                  "defaultAcademy",
                  JSON.stringify(lstAcademy[highlightAcademy])
                );
                dispatch({
                  type: siteActionType.PICK_DEFAULT_ACADEMY,
                });
        ...
        {lstAcademy.length > 0 && (
          <div className="service">
            <div className="weekly-training">
              ...
              {lstAcademy[highlightAcademy].weeklyTraining?.class.map(
                (item, index) => (
                  <WeeklyTrainingItem data={item} key={index} index={index} />
                )
              )}
            </div>
                ...
                >
                  ...
                  <div className="wrap-row other-location">
                    <div>
                      <b>{item.ms_name}</b>
                      <label>
                        {Utils.showDistance(lstAcademy[index].distance)}
                        {` miles away`}
                      </label>
                    </div>
                    <FontAwesomeIcon
